...
public class CustomJavaCodegen extends DefaultCodegen implements CodegenConfig {
	...
	protected String developerName = "Fiorin Davide";

	protected String developerEmail = "fiorindavide@gmail.com";
...
//        cliOptions.add(CliOption.newBoolean(DISABLE_HTML_ESCAPING, "Disable HTML escaping of JSON strings when using gson (needed to avoid problems with byte[] fields)"));
	...
	public void processOpts () {
		...
		if (additionalProperties.containsKey(CodegenConstants.INVOKER_PACKAGE)) {
			...
			String derviedInvokerPackage = deriveInvokerPackageName(
		...
		if (additionalProperties.containsKey(CodegenConstants.SERIALIZE_BIG_DECIMAL_AS_STRING)) {
			this.setSerializeBigDecimalAsString(
				Boolean.valueOf(additionalProperties.get(CodegenConstants.SERIALIZE_BIG_DECIMAL_AS_STRING).toString()));
		}
	...
	public String toVarName (String name) {
		...
		// sanitize name
		name = sanitizeName(name); // FIXME: a parameter should not be assigned. Also declare the methods parameters as
									// 'final'.
	...
	public String toModelName (final String name) {
		...
		if (camelizedName.matches("^\\d.*")) {
			...
			LOGGER
				.warn(name + " (model name starts with number) cannot be used as model name. Renamed to " + modelName);
			return modelName;
		}
	...
	public CodegenModel fromModel (String name, Model model, Map<String, Model> allDefinitions) {
		...
		if (allDefinitions != null && codegenModel.parentSchema != null && codegenModel.hasEnums) {
			...
			codegenModel = CustomJavaCodegen.reconcileInlineEnums(codegenModel, parentCodegenModel);
		}
		return codegenModel;
	}
...
//		}
	...
	/*
	 * @Override public String findCommonPrefixOfVars(List<String> vars) { String prefix =
	 * StringUtils.getCommonPrefix(vars.toArray(new String[vars.size()])); // exclude trailing characters that should be
	 * part of a valid variable // e.g. ["status-on", "status-off"] => "status-" (not "status-o") return
	 * prefix.replaceAll("[a-zA-Z0-9]+\\z", ""); }
	 */
